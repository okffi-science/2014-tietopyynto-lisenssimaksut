...
```{r costs3b, cache=FALSE, echo=FALSE, message=FALSE, warning=FALSE, fig.width=20, fig.height=10}
d2 <- d
dsub <- d2 %>% select("Country", "Year", "Publisher", "Cost") %>%
              filter(Publisher %in% selected.publishers) %>%
	      group_by(Country, Year, Publisher) %>%
...
`r figs("costs3b", "Same as above but all publishers included. Perhaps we like to include this figure instead and just discuss the issues that affect the comparison ie. latter years have more complete lists of publishers etc.")`.
...
for (varname in vars) {
  ...
  p <- ggplot(df, aes(x = ranking, y = Cost, color = Country)) + geom_point() + labs(y = "Cost (EUR)", x = varname, title = varname) + scale_y_log10() + facet_grid(. ~ Publisher)
  print(p)
}
