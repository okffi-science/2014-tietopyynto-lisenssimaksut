...
for (varname in vars) {
  ...
  bonferroni <- length(unique(df$Country)) * length(vars)
  
  #vec <- c(spearman = unname(r$estimate), spearman.p = r$p.value,
  #    lm.slope = coef(lmres)[["ranking"]], lm.p = anova(lmres)["ranking", "Pr(>F)"])
      
  for (country in unique(df$Country)) {
  
    dfsub <- subset(df, Country == country)
    r <- cor.test(log10(dfsub$Cost), dfsub$ranking, method = "spearman", use = "pairwise.complete.obs")
    lmres <- lm(log10(1 + Cost) ~ ranking, data = dfsub)
    vec[[paste("spearman", country, sep = "/")]] = unname(r$estimate)
    vec[[paste("spearman.p", country, sep = "/")]] = unname(r$p.value)/bonferroni
    vec[[paste("lm.slope", country, sep = "/")]] = coef(lmres)[["ranking"]]
    vec[[paste("lm.p", country, sep = "/")]] = anova(lmres)["ranking", "Pr(>F)"]/bonferroni
...
kable(t(res), digits = 2)
```

`r tbls("ranking1tab", "University ranking (Times ranking) variables versus subscription costs with selected publishers. Spearman correlation (rho and p) and linear model (slope and p). The p-values are adjusted for multiple testing with Bonferroni correction.")`
